...
dict(slug="mentions-legales", title="Mentions légales de Comparépargne",
 ...
 blocks=[
  ...
  ("h2","Crédits photo"),
  ("p","Les photographies publiées sur ce site proviennent d'Unsplash et sont utilisées dans les conditions de la licence Unsplash. La liste des visuels et le lien vers chaque photographie d'origine sont tenus à jour dans le fichier <code>sources_photos.json</code> du site. Les logos des assureurs affichés dans les classements et les comparatifs sont la propriété de leurs titulaires. Ils sont reproduits à seule fin d'identification des contrats comparés, sans lien de partenariat ni d'approbation, et sont retirés sur simple demande du titulaire à l'adresse de contact ci-dessus. Le fichier <code>sources_logos.json</code> du site conserve l'origine de chaque logo."),
