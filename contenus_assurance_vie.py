...
dict(
 ...
 brief=[
  ...
  "Un taux de fonds en euros annoncé n'est pas toujours servi : six contrats du panel le conditionnent à une part minimale d'unités de compte, ce que ni Groupama Modulation ni Gan Patrimoine ne font",
 ],
 table=dict(
  ...
  rows=[
   ...
   ["MACSF RES","7,5","0 %","0,80 %","Environ 60","300 €","Un fonds en euros correct",0],
   ["MAIF Responsable","7,4","0 %","0,90 %","Environ 50","100 €","La gamme entièrement labellisée",0],
   ["Yomoni Vie","7,2","0 %","0,60 % + mandat","Gestion pilotée","1 000 €","La gestion déléguée lisible",0],
 ...
 sections=[
  ...
  dict(h2="Le tableau comparatif des 16 contrats", id="tableau", body=[
   ...
   ("p","Trois enseignements ressortent. D'abord, la ligne de partage ne passe plus entre les contrats en ligne et les contrats de réseau : Groupama Modulation et Gan Patrimoine ne prélèvent rien sur les versements, comme les meilleurs contrats en ligne. Ensuite, le nombre de supports n'est pas un critère en soi, mille supports mal documentés valent moins que deux cent cinquante fonds dont les frais courants sont affichés. Enfin, le ticket d'entrée n'est plus un obstacle : la moitié du panel s'ouvre à 300 € ou moins."),
  ]),
  dict(h2="Ce que coûtent vraiment les frais sur huit ans", id="frais", body=[
   ...
   ("table2",dict(
     ...
     rows=[["Groupama Modulation","0 %","0,60 %","24 170 €","1 180 €"],
           ["Contrat en ligne le moins cher","0 %","0,50 %","24 350 €","1 000 €"],
   ...
   ("p","L'écart final atteint 1 650 € entre le premier et le dernier profil, soit plus de 8 % du capital de départ, pour un rendement identique. Groupama Modulation se place à 180 € du contrat le moins cher du marché, un écart que son fonds en euros compense sur le premier exercice. C'est le seul paramètre de l'équation que vous maîtrisez complètement le jour de la souscription."),
  ...
  dict(h2="Quel contrat pour quel profil", id="profils", body=[
   ...
   ("p","Groupama Modulation, dont le fonds en euros est servi au-dessus de la moyenne du marché avec une garantie pleine du capital, nette de frais de gestion. Vérifiez toujours ce niveau de garantie : plusieurs fonds de nouvelle génération du panel ne garantissent que 98 % du capital, ce qui change la nature du support."),
...
dict(
 ...
 sections=[
  dict(h2="Les trois circuits pour ouvrir une assurance vie", id="circuits", body=[
   ...
   ("podium",None),
   ("p","Le même assureur peut donc servir un contrat à 0 % de frais d'entrée dans un circuit et un contrat à 3 % dans un autre, avec des gammes de supports différentes. Comparer des assureurs sans regarder le contrat exact, c'est comparer des noms de marques. C'est ce qui explique la première place de l'assureur en direct : sur Groupama Modulation, le contrat distribué par le réseau affiche les mêmes frais qu'un contrat en ligne, l'accompagnement en plus."),
  ]),
  dict(h2="Le tableau comparatif des trois circuits", id="tableau", body=[
   ("table",None),
   ("p","L'écart de frais d'entrée est le point le plus visible, mais il n'est pas le plus coûteux. Trois pour cent prélevés une fois sur un versement de 10 000 €, c'est 300 €. Un quart de point de frais de gestion supplémentaire sur le même montant pendant vingt ans, c'est plus du double. Sur les deux lignes, l'assureur en direct et le courtier en ligne sont désormais au même niveau, seul le guichet bancaire reste en retrait."),
  ...
  dict(h2="Les erreurs les plus fréquentes du premier contrat", id="erreurs", body=[
   ("ul",[
    "<b>Signer les frais d'entrée affichés d'un contrat bancaire</b> sans les discuter, alors que d'autres circuits ne prélèvent plus rien du tout",
...
dict(
 ...
 sections=[
  ...
  dict(h2="Comment répartir 20 000 euros", id="repartition", body=[
   ...
   ("p","Le raisonnement tient en une phrase : on ne place jamais en unités de compte un euro dont on peut avoir besoin dans les trois ans, et on ne laisse jamais dormir sur un livret un euro dont on est sûr de ne pas avoir besoin avant dix ans. Sur la poche intermédiaire, celle qui vise trois à cinq ans, un fonds en euros servi au-dessus de la moyenne du marché comme celui de Groupama Modulation fait mécaniquement mieux qu'un livret plafonné."),
 ...
 verdict=dict(h2="Notre verdict", body=[
  "Gardez le <b>Livret A</b> pour l'épargne de précaution, trois à six mois de dépenses courantes, et rien de plus : au-delà, son plafond et son taux réglementé le rendent moins intéressant qu'un bon fonds en euros.",
